...
### 6. Smooth tempo jumps between consecutive songs — *medium impact / medium effort*
**Hats: UXR, PM (corrected).** *The original finding's premise was wrong.* The app does **not** swap songs on BPM micro-spikes: the auto-loop only picks a new track when the current one ends (`AutoModeService.tick`, `:207–208`, `remaining < END_OF_TRACK_THRESHOLD_MS` = 8s) or playback goes idle. The 5s poll just refreshes the now-playing countdown, and the signal is already 10s-smoothed (`HeartRateRelay`) before it becomes a target BPM. So songs change at the natural cadence of track length (~once per song), not per poll, and there is no mid-song churn. The real, milder concern: at each track boundary a large effort change since the last boundary can produce a jarring **tempo jump** between consecutive songs. Worth exploring clamping how far the next pick's target BPM may move from the just-finished track, and/or phase-aware targeting (warmup → steady → sprint → cool-down) for *what* BPM to aim at — not for switch frequency.
...
- **Add retry + user-facing feedback to Wearable Data Layer sends** `[TL, UXD, PM]` ✅ *(retry done)* — New `WearMessenger.send()` centralizes node-lookup+send for the one-shot control/config messages with exponential backoff (3 attempts, 400ms→3s, matching `SpotifyClient`) and records a single dropout counter on final failure. `AutoModeService.sendWatchCommand` and `WatchSync.sendSignalSource` now delegate to it. **Deliberate exclusion:** `HeartRatePublisher.publish` (the ~1Hz HR/cadence stream) is left fire-and-forget — a dropped sample self-heals on the next reading, and a backed-off retry would deliver a stale value out of order. **Still open:** the "Watch disconnected" UI state — folded into the `HomeScreen` connection-status item below (the new `watch_unreachable`/`watch_send_failed` counters give it something to bind to).
- **Handle sensor loss gracefully in the auto-loop** `[TL, PM]` ✅ — `DEFAULT_HR=80` is gone. The loop now evaluates signal freshness (`SIGNAL_FRESH_MS` = 12s) on **every poll** via `evaluateSignal()` — not just at picks — using a time-based "lost duration" from `lastLiveSignalAtMs`. Idle + lost → "Waiting for signal from watch…" (polls every 5s to resume quickly). Playing + lost → keeps the current track and shows "Signal lost — keeping current track" without interrupting it. After `SIGNAL_GIVE_UP_MS` (120s) lost it stops auto mode via the normal `ACTION_STOP` path with a "No signal from watch — auto mode stopped" message (survives teardown, shown on `HomeScreen`) — *regardless of whether Spotify is still playing*. `signal_loss` telemetry records each wait/keep_playing/stop decision. Verified end-to-end on the emulator (incl. keep_playing → stop at 120s while Spotify kept playing, clean teardown). The freshness/give-up decision is extracted into a pure `SignalFreshness` helper covered by unit tests (`SignalFreshnessTest`, 9 cases) — the first real tests in the repo, and a starting point for the P2 auto-loop test item below.
...
- **A/B test between-song tempo-jump smoothing** `[UXR]` — *Corrected: poll interval does not drive song changes — songs change only at track end (`AutoModeService.tick`), so a 5s-vs-60s-polling test measures the wrong variable.* The meaningful experiment is clamping how far the next track's target BPM may move from the just-finished track (vs. no clamp); track skip rate, session length, and satisfaction.
...
3. **Stop failing silently on watch/sensor loss** — ✅ *sensor-loss escalation done* (auto-loop waits then stops instead of defaulting to HR 80) and ✅ *retry+backoff on Data Layer sends done* (`WearMessenger`). Still open: a connection/freshness indicator on `HomeScreen` (P1, all four hats).
