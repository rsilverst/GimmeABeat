...
- **Handle sensor loss gracefully in the auto-loop** `[TL, PM]` ✅ — `pickAndPlay` no longer fabricates `DEFAULT_HR=80` (constant removed). It now tracks signal freshness (`SIGNAL_FRESH_MS` = 12s) and `consecutiveSignalMisses`: brief gaps coast on the last known reading; after 3 misses it shows "Waiting for signal from watch…" and the loop polls every 5s to resume quickly; after ~2 min of no signal it stops auto mode via the normal `ACTION_STOP` path and leaves a "No signal from watch — auto mode stopped" message (survives teardown, shown on `HomeScreen`). New `signal_loss` telemetry records each wait/stop decision. Follow-up: a pure, unit-testable extraction of the freshness/miss decision (folds into the P2 auto-loop test item).
