...
3. **Stop failing silently on watch/sensor loss** — ✅ *escalate-after-consecutive-misses done* (auto-loop now waits then stops instead of defaulting to HR 80). Still open: retry+backoff on Data Layer **sends**, and a connection/freshness indicator on `HomeScreen` (P1, all four hats).
